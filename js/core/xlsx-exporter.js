...
class XlsxExporter {
  ...
  static sheetXml(rows, previewSheet = null) {
    ...
    const cols = Array.from({ length: count }, (_, i) => `<col min="${i + 1}" max="${i + 1}" width="${i === 0 ? 13 : 15}" customWidth="1"/>`).join('');
  ...
  static polarScatterChartXml(sheetName, rows) {
    ...
    const title = text => `<c:title><c:tx><c:rich><a:bodyPr/><a:lstStyle/><a:p><a:r><a:rPr lang="en-US"/><a:t>${text}</a:t></a:r></a:p></c:rich></c:tx><c:layout/></c:title>`;
    return `<?xml version="1.0" encoding="UTF-8"?><c:chartSpace xmlns:c="http://schemas.openxmlformats.org/drawingml/2006/chart" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main"><c:chart>${title('Polar coordinate preview')}<c:plotArea><c:layout/><c:scatterChart><c:scatterStyle val="lineMarker"/><c:varyColors val="0"/>${series}<c:axId val="1"/><c:axId val="2"/></c:scatterChart><c:valAx><c:axId val="1"/><c:scaling><c:orientation val="minMax"/></c:scaling><c:delete val="0"/><c:axPos val="b"/>${title('X = I cos(theta) [Counts]')}<c:majorGridlines/><c:numFmt formatCode="0" sourceLinked="0"/><c:crossAx val="2"/><c:crosses val="autoZero"/></c:valAx><c:valAx><c:axId val="2"/><c:scaling><c:orientation val="minMax"/></c:scaling><c:delete val="0"/><c:axPos val="l"/>${title('Y = I sin(theta) [Counts]')}<c:majorGridlines/><c:numFmt formatCode="0" sourceLinked="0"/><c:crossAx val="1"/><c:crosses val="autoZero"/></c:valAx></c:plotArea><c:legend><c:legendPos val="r"/><c:layout/></c:legend><c:plotVisOnly val="1"/></c:chart></c:chartSpace>`;
  }
